...
- 🛰️ **Companion (optional)** — install the small **[ComfyUI companion extension](https://github.com/ethanfel/ComfyUI-ComfyPeeper-compagnion)** to **Send to ComfyUI** (load a workflow straight into your live editor tab, not just queue it headless) and **Send to Discord** (right-click an output image in ComfyUI → posts it to a webhook). The plugin auto-detects companion-enabled servers and labels them by name (advanced mode).
...
- 🔁 **Repost** — right-click any image/video → **Repost to…** to re-post it to another server/channel **as you** (a clean message, no "Forwarded" tag), with the workflow still embedded. Pick a favourite channel (configured in settings) or search for any channel you can post to. Pairs nicely with the companion's *Send to Discord* (stage in a private channel, repost where you want). Set a **drop channel** in settings and a small **inbox** pops up bottom-right whenever an image lands there — repost it in one click without ever opening the channel.
